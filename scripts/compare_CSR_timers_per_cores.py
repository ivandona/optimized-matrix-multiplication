...
# Load CSV
df = pd.read_csv("results_CSR_reverse_pack.csv")
...
# Calcola fattore di normalizzazione: cores / max_cores
max_cores = best_per_core["cores"].max()
scaling_factors = best_per_core["cores"] / max_cores

# Componenti da usare
components = ["comm_dist", "computation_sort", "computation_mult", "comm_aggr"]

# Applica normalizzazione
normalized_components = best_per_core[components].multiply(scaling_factors.values, axis=0)
...
bottom = np.zeros(len(normalized_components))

# Disegna barre impilate
for comp in components:
    heights = normalized_components[comp].values
