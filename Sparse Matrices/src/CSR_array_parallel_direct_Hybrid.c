//##############################################################################################################################
// Matrices Multiplication in format CSR using an array as representation (Hybrid direct version)
//##############################################################################################################################
//###############################################################
// Libraries
//###############################################################
...
//---------------------------------------------------------------
void comms_data_distribution(int** A_row_ptr, int** A_col, double** A_value, int** B_row, int** B_col_ptr, double** B_value, int local_rows, const int nnz, const int n, const int world_rank, const int world_size, const int num_threads){
    ...
    if(world_rank==0){
        ...
        for(i=0; i<world_size; i++){
            ...
            A_displs[i]=(*A_row_ptr)[rows_offset[i]];
        }
        //set counts arrays
...
//---------------------------------------------------------------
double* multiply_sparse_CSC_x_CRS_threads(int* A_row_ptr, int* A_col, double* A_value, int* B_row, int* B_col_ptr, double* B_value, const int local_rows, const int n, const int num_threads){
...
int main(int argc, char** argv){
    ...
    start = MPI_Wtime();
    comms_data_distribution(&A_row_ptr, &A_col, &A_value, &B_row, &B_col_ptr, &B_value, local_rows, nnz, n, world_rank, world_size, num_threads);
    end = MPI_Wtime();
    ...
    //########################
    start = MPI_Wtime();
    double* C_partial = multiply_sparse_CSC_x_CRS_threads(A_row_ptr, A_col, A_value, B_row, B_col_ptr, B_value, local_rows, n, num_threads);
    ...
    if(world_rank == 0){
        printf("CSR array parallel direct Hybrid (n=%d, d=%.2f)[#pr=%d][#th=%d]\n", n, density_percentage/100, world_size, num_threads);
        ...
        char filename[128];
        if(argc==3){
            snprintf(filename, sizeof(filename),"res/%s/output_CSR_apd_Hybrid_%d_%d.csv", argv[2], world_size, num_threads);
        }
        else snprintf(filename, sizeof(filename),"res/output_CSR_apd_Hybrid_%d_%d.csv", world_size, num_threads);
        FILE *fp = fopen(filename, "a");
        if (fp != NULL){
            fprintf(fp,
                "%s,%d,%.2f,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
                "CSR_apd_Hybrid",         // algorithm
                n,                        // matrix dimension
