//##############################################################################################################################
// Matrices Multiplication in format CSR using an array as representation (Hybrid reverse version)
//##############################################################################################################################
//###############################################################
// Libraries
//###############################################################
...
//---------------------------------------------------------------
void comms_data_distribution(int** A_row, int** A_col_ptr, double** A_value, int** B_row_ptr, int** B_col, double** B_value, int local_rows, const int nnz, const int n, const int world_rank, const int world_size, const int num_threads){
    ...
    if(world_rank==0){
        ...
        for(i=0; i<world_size; i++){
            rows_offset[i] = n - (world_size-i)*rows_per_process;
            rows_distr[i]=rows_per_process;
            if(i<n%world_size){
                rows_offset[i] = i*(rows_per_process+1);
                rows_distr[i]++;
            }
            A_displs[i]=(*A_col_ptr)[rows_offset[i]];
            B_displs[i]=(*B_row_ptr)[rows_offset[i]];
        }

        for(i=1; i<world_size; i++){ //better to keep it serial, few operations and relatively small world_size (<=256)
            A_counts[i-1]=A_displs[i]-A_displs[i-1];
            B_counts[i-1]=B_displs[i]-B_displs[i-1];
        }
        A_counts[world_size-1] = nnz-A_displs[world_size-1];
        B_counts[world_size-1] = nnz-B_displs[world_size-1];
    }
...
//---------------------------------------------------------------
double* multiply_sparse_CSC_x_CRS_threads(int* A_row, int* A_col_ptr, double* A_value, int* B_row_ptr, int* B_col, double* B_value, const int rows, const int n, const int num_threads){
...
int main(int argc, char** argv){
    ...
    if(world_rank == 0){
        start = MPI_Wtime();
        convert_CSR_to_CSC_threads(A_row_ptr, A_col, &A_value, &A_col_ptr, &A_row, n, nnz, num_threads);
        end = MPI_Wtime();
    ...
    start = MPI_Wtime();
    comms_data_distribution(&A_row, &A_col_ptr, &A_value, &B_row_ptr, &B_col, &B_value, local_rows, nnz, n, world_rank, world_size, num_threads);
    end = MPI_Wtime();
    ...
    //########################
    start = MPI_Wtime();
    double* C_partial = multiply_sparse_CSC_x_CRS_threads(A_row, A_col_ptr, A_value, B_row_ptr, B_col, B_value, local_rows, n, num_threads);
    ...
    if(world_rank == 0){
        printf("CSR array parallel reverse Hybrid (n=%d, d=%.2f)[#pr=%d][#th=%d]\n", n, density_percentage/100, world_size, num_threads);
        ...
        char filename[128];
        if(argc==3){
            snprintf(filename, sizeof(filename),"res/%s/output_CSR_apr_Hybrid_%d_%d.csv", argv[2], world_size, num_threads);
        }
        else snprintf(filename, sizeof(filename),"res/output_CSR_apr_Hybrid_%d_%d.csv", world_size, num_threads);
        FILE *fp = fopen(filename, "a");
        if (fp != NULL){
            fprintf(fp,
                "%s,%d,%.2f,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
                "CSR_apr_Hybrid",         // algorithm
                n,                        // matrix dimension
