//##############################################################################################################################
// Matrices Multiplication in format CSR using an array as representation (OpenMP reverse version)
//##############################################################################################################################
//###############################################################
// Libraries
//###############################################################
...
//---------------------------------------------------------------
double* multiply_sparse_CSC_x_CRS_threads(int* A_row, int* A_col_ptr, double* A_value, int* B_row_ptr, int* B_col, double* B_value, const int n, const int num_threads){
...
int main(int argc, char** argv){
    ...
    //########################
    start = omp_get_wtime();
    double* C = multiply_sparse_CSC_x_CRS_threads(A_row, A_col_ptr, A_value, B_row_ptr, B_col, B_value, n, num_threads);
    ...
    printf("CSR array parallel reverse OpenMP (n=%d, d=%.2f)[#th=%d]\n", n, density_percentage/100, num_threads);
    ...
    char filename[128];
    if(argc==3){
        snprintf(filename, sizeof(filename),"res/%s/output_CSR_apr_OpenMP_%d_%d.csv", argv[2], 1, num_threads);
    }
    else snprintf(filename, sizeof(filename),"res/output_CSR_apr_OpenMP_%d_%d.csv", 1, num_threads);
    FILE *fp = fopen(filename, "a");
    if (fp != NULL){
        fprintf(fp,
            "%s,%d,%.2f,1,%d,%.3f,%.3f,0,0,0,%.3f,%.3f,%.3f,%.3f\n",
            "CSR_apr_OpenMP",         // algorithm
            n,                        // matrix dimension
